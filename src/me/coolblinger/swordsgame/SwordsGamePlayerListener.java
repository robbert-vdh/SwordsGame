...
public class SwordsGamePlayerListener extends PlayerListener {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (plugin.define.containsKey(player)) {
			...
			} else if (plugin.define.get(player).mode == "setspawn") {
				if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (event.getClickedBlock() != null) {
						Vector vector = event.getClickedBlock().getLocation().toVector();
						String arenaName = plugin.getArena(vector);
						if (arenaName != null) {
							int setSpawn = plugin.arenas.get(arenaName).setSpawn(vector);
							if (setSpawn != 0) {
								player.sendMessage(ChatColor.GREEN + "Added spawn " + ChatColor.WHITE + setSpawn + ChatColor.GREEN + " out of " + ChatColor.WHITE + "4" + ChatColor.GREEN + " for arena '" + ChatColor.WHITE + arenaName + ChatColor.GREEN + "'.");
							} else {
